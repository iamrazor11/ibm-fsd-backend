...
public class EmpService implements Callable<Object>{
	...
	Dao d=(Dao) context.getBean("passDao");
...
//		Employee newEmp = new Employee(nId, nName, nDesign, nDept, nCountry);
		//empList.add(newEmp);
		Employee newEmp=(Employee)context.getBean("passUpdateEmployee");
		d.update(x,newEmp);
	}
	...
	public void viewAll() {
		//empList.forEach(e->{
		//	System.out.println(e);
			List<Employee> eList=d.displayAll();
			//eList.forEach(e->{System.out.println(e.getId()+" "+e.getName()+" ");});
